...
- Synced with: statusline.sh v0.39.0
...
strip applies it to 1h cells (1800). A future cell is a slot, never a
verdict: a dry projection does not overwrite cells (the wall belongs to
the notices, which state the exact time; only statusline's folded token
still carries the `×` mark). The dim count may sit one off the
sentence's `~N awake`: cells are grid slots, the sentence is clocks, the
same one-cell tolerance the fold count carries.
